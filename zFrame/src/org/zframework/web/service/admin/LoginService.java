...
import org.zframework.web.controller.admin.RoleController;
		...
		if(user!=null){
			...
			if(user.getRoles().size()>0){
				...
				for(Role role : user.getRoles()){
					if(role.getName().equals(ApplicationCommon.DATADICT.get("SystemRole"))){
						isSystemRole = true;
						break;
					}
				...
				if(isSystemRole){//系统管理员拥有全部资源权限，所有按钮权限
					user.setResources(baseDao.list(Resource.class, Restrictions.eq("enabled", 0),new Order[]{Order.asc("location"),Order.asc("parentId")}));
